...
router.get('/quizzes', (req, res) => {
  ...
  const jsonLd = [
    breadcrumbLd([{ name: 'Home', path: '/' }, { name: 'Quizzes', path: '/quizzes' }]),
    {
      ...
      name: 'Quizzes', url: absUrl('/quizzes'),
...
router.get('/quizzes/:id/:slug?', optionalAuth, (req, res, next) => {
  ...
  const seoDescriptor = resolveSeo(s, {
    ...
    title: row.title,
    description: row.description || `Take the "${row.title}" ${compat ? 'compatibility ' : ''}quiz on ${SITE_NAME}.`,
...
router.get('/how-it-works', (req, res) => {
  const steps = [
    ['Create your account', 'Sign up with a username and verify your email with a one-time code. Your email is never shown to anyone else. Joining with a friend’s referral link gets you 2 points to start.'],
    ['Build your profile', 'Add your gender, date of birth and country, then pick your state and city, write a few words about yourself and choose up to 10 areas of interest. Add a display picture and a gallery of up to 25 photos and short reels (videos up to 1 minute).'],
  ...
  renderInfo(res, {
    ...
    title: 'How it works',
    description: `How ${SITE_NAME} works — build a profile around your interests, meet like-minded people on the Highway, take quizzes, vote in polls, chat in real time and earn points on the leaderboard. Free to join and open to everyone.`,
